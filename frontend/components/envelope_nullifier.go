...
func (c *EnvelopeNullifier) Render() vecty.ComponentOrHTML {
	...
	if c.Unavailable {
		return elem.Div(
			...
			elem.Main(elem.Heading2(vecty.Text("This envelope does not exist")), elem.Heading4(vecty.Text("It can take up to 10 minutes for your vote to register on the blockchain once it has been cast."))),
		)
	}
	...
	return elem.Div(
		vecty.Markup(vecty.Attribute("id", "main")),
		renderServerConnectionBanner(),
		elem.Main(elem.Heading2(vecty.Text("Loading envelope..."))),
	)
}
